...
inline bool imageWriterToolExists(std::string const &toolName)
{
    ...
    while (std::getline(pathStream, directory, ':')) {
        ...
        std::ifstream candidate((directory + "/" + toolName).c_str());
        if (candidate.good()) {
            return true;
        }
    }
...
inline std::string imageMagickProgram()
{
    if (imageWriterToolExists("magick")) {
        return "magick";
    }
    return "convert";
}
...
void ImageWriter<T>::imageMagickPpmToGif(std::string const &fName) const
{
    if (global::mpi().isMainProcessor()) {
        ...
        if (command == "magick") {
            convCommand = command + " " + global::directories().getImageOutDir() + fName + ".ppm "
...
void ImageWriter<T>::imageMagickResize(std::string const &fName, plint sizeX, plint sizeY) const
{
    ...
    if (global::mpi().isMainProcessor()) {
        ...
        if (command == "magick") {
            imStream << command << " " << global::directories().getImageOutDir() << fName
                     << ".ppm -resize " << sizeX << "x" << sizeY << " "
                     << global::directories().getImageOutDir() << fName << ".gif";
        } else {
            imStream << command << " -resize " << sizeX << "x" << sizeY << " "
