...
struct FreeSurfaceFields3D {
        ...
        container(0)
    {
        ...
        // setToConstant(flag, flag.getBoundingBox(), (int)freeSurfaceFlag3D::empty);
...
const int FreeSurfaceFields3D<T, Descriptor>::envelopeWidth =
    4;  // Necessary when we use height functions to compute the curvature,
...
template <typename T, template <typename U> class Descriptor>
const int FreeSurfaceFields3D<T, Descriptor>::smallEnvelopeWidth = 4;
...
public:
    ...
    Actions3D immersedWallActions(
        plint numIBIterations,MultiContainerBlock3D& container, std::vector<Array<T, 3> > const &vertices,
        std::vector<T> const &areas, std::vector<int> const &flags, VelFunction velFunction,
        bool strongRepelling)
    {
        Actions3D actions;
