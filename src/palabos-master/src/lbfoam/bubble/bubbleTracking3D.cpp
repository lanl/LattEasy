 ...
 *
 */
#include "lbfoam/bubble/bubbleTracking3D.h"
...
BubbleTracking3D::BubbleTracking3D(MultiBlock3D &templ)
    : bubbleContainer(
          createContainerBlock(templ, new BubbleCounterData3D(maxNumBubbles))),
      bubbleAnalysisContainer(
          createContainerBlock(templ, new BubbleAnalysisData3D())),
      bubbleRemapContainer(
          createContainerBlock(templ, new BubbleRemapData3D(maxNumBubbles))),
      mpiData(*bubbleContainer),
      tagMatrix(new MultiScalarField3D<plint>(*bubbleContainer)),
      disjoiningPressureField(
          new MultiScalarField3D<double>(*bubbleContainer)) {
  setToConstant(*tagMatrix, tagMatrix->getBoundingBox(), (plint)-1);
  tagMatrix->setMultiBlockManagement().changeEnvelopeWidth((plint)4);
}

BubbleTracking3D::~BubbleTracking3D() {
...
pluint BubbleTracking3D::countAndTagBubbles() {
  std::vector<MultiBlock3D *> args;
  args.push_back(tagMatrix);
  args.push_back(bubbleRemapContainer);
  applyProcessingFunctional(new CollectBubbleTags3D(),
                            bubbleRemapContainer->getBoundingBox(), args);
  plint numBubbles = globalBubbleIds();
  applyProcessingFunctional(new ApplyTagRemap3D(),
                            bubbleRemapContainer->getBoundingBox(), args);
  return numBubbles;
}

void BubbleTracking3D::computeBubbleData(pluint numBubbles) {
  ...
  for (pluint i = 0; i < numBubbles; ++i) {
    bubbleCenter[i] =
        Array<double, 3>(bubbleCenterX[i], bubbleCenterY[i], bubbleCenterZ[i]);
...
plint BubbleTracking3D::globalBubbleIds() {
  ...
  for (pluint i = 0; i < localIds.size(); ++i) {
    ...
    BubbleRemapData3D *pData =
        dynamic_cast<BubbleRemapData3D *>(atomicDataContainer.getData());
    PLB_ASSERT(pData);
...
void BubbleTracking3D::resetBubbleContainer() {
  ...
  for (; it != domains.end(); ++it, ++pos) {
    ...
    if (threadAttribution.isLocal(id)) {
      ...
      dynamic_cast<BubbleCounterData3D *>(atomicDataContainer.getData())
          ->reset();
    }
  }
}

void BubbleTracking3D::bubbleBucketFill(MultiScalarField3D<int> &flag) {
  setToConstant(*tagMatrix, tagMatrix->getBoundingBox(), (plint)-1);
...
bool BubbleCounterData3D::convertCell(
    plint &tag0, plint tag1, plint tag2, plint tag3, plint tag4, plint tag5,
    plint tag6, plint tag7, plint tag8, plint tag9, plint tag10, plint tag11,
    plint tag12, plint tag13, plint tag1_, plint tag2_, plint tag3_,
    plint tag4_, plint tag5_, plint tag6_, plint tag7_, plint tag8_,
...
void CountBubbleIteration3D::processGenericBlocks(
  ...
  BubbleCounterData3D *pData =
      dynamic_cast<BubbleCounterData3D *>(dataBlock.getData());
  PLB_ASSERT(pData);
...
void CollectBubbleTags3D::processGenericBlocks(
  ...
  AtomicContainerBlock3D *pDataBlock =
      dynamic_cast<AtomicContainerBlock3D *>(atomicBlocks[1]);
  PLB_ASSERT(pDataBlock);
  AtomicContainerBlock3D &dataBlock = *pDataBlock;
  BubbleRemapData3D *pData =
      dynamic_cast<BubbleRemapData3D *>(dataBlock.getData());
  PLB_ASSERT(pData);
  BubbleRemapData3D &data = *pData;
...
void ApplyTagRemap3D::processGenericBlocks(
    Box3D domain, std::vector<AtomicBlock3D *> atomicBlocks) {
  PLB_ASSERT(atomicBlocks.size() == 2);
  ScalarField3D<plint> *pTagMatrix =
      dynamic_cast<ScalarField3D<plint> *>(atomicBlocks[0]);
