 ...
 *
 */
#ifndef BUBBLE_TRACKING_3D_HH
#define BUBBLE_TRACKING_3D_HH
...
void BubbleTracking3D::calculateBubbleGrowth(
...
void BubbleTracking3D::calculateDisjoiningPressure(
...
template <typename T>
void BubbleTracking3D::bubbleAnalysis(MultiScalarField3D<int> &flag,
      ...
      for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
        ...
        if (tag >= 0) {
          if (flagMatrix.get(iX + flagOffset.x, iY + flagOffset.y,
                             iZ + flagOffset.z) == freeSurfaceFlag::empty) {
...
template <typename T, template <typename U> class Descriptor>
void BubbleTracking3D::stabilizeLattice(
