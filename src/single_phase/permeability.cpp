  ...
  while (std::getline(pathStream, directory, pathSeparator)) {
    ...
    for (std::vector<std::string>::const_iterator it = suffixes.begin();
      ...
      std::ifstream candidate(
          (directory + dirSeparator + toolName + *it).c_str());
      if (candidate.good()) {
        return true;
      }
    }
...
public:
  ...
  void operator()(plint iX, plint iY, plint iZ, T &density,
...
void readGeometry(const std::string &inputFolder,
                  MultiScalarField3D<int> &geometry, bool vtk_out,
  ...
  for (plint iX = 0; iX < nx - 1; ++iX) {
    if (!geometryFile.is_open()) {
      pcout << "Error: could not open the geometry file " << geometryPath
            << std::endl;
      exit(EXIT_FAILURE);
    }

    geometryFile >> *slice;
    copy(*slice, slice->getBoundingBox(), geometry,
         Box3D(iX, iX, 0, ny - 1, 0, nz - 1));
  }
  ...
  //{
  // std::auto_ptr<MultiScalarField3D<T> > floatTags =
  // copyConvert<int,T>(geometry, geometry.getBoundingBox()); std::vector<T>
  // isoLevels; isoLevels.push_back(0.5); typedef TriangleSet<T>::Triangle
  // Triangle; std::vector<Triangle> triangles; Box3D domain =
  // floatTags->getBoundingBox().enlarge(-1); domain.x0++; domain.x1--;
...
void computePermeability(MultiBlockLattice3D<T, DESCRIPTOR> &lattice, T nu,
                         T deltaP, Box3D domain, T &perm, T &meanU,
                         bool printPerm = true) {
  ...
  meanU =
      computeAverage(*computeVelocityComponent(lattice, domain1, xComponent));
...
int main(int argc, char **argv) {
  ...
  if (!gifOutputEnabled) {
    pcout
        << "Skipping GIF previews because ImageMagick `convert` was not found."
        << std::endl;
  }

  MultiBlockLattice3D<T, DESCRIPTOR> lattice(
      nx, ny, nz, new BGKdynamics<T, DESCRIPTOR>(omega));
