...
//  //    vtkOut.writeData<3, float>(*adv, "adVelocity");
  ...
  //                            1.0);
    ...
    std::unique_ptr<MultiScalarField3D<T>> VOF(lbmSmoothen<T, DESCRIPTOR>(
...
//  }
  ...
  setToConstant(fields.flag, lateral1, (int)freeSurfaceFlag::wall);
  setToConstant(fields.flag, lateral2, (int)freeSurfaceFlag::wall);
  setToConstant(fields.flag, lateral3, (int)freeSurfaceFlag::wall);
  setToConstant(fields.flag, lateral4, (int)freeSurfaceFlag::wall);
      ...
      for (plint iY=0; iY<param.ny-1; ++iY){
          for (plint iZ=0; iZ<param.nz-1; ++iZ){
              if (geometry.get(iX,iY, iZ) == 0) {
                      ...
                      setToConstant(fields.flag, porePoint, (int)freeSurfaceFlag::fluid);
                    }
              if (geometry.get(iX,iY, iZ) == 1) {
                  Box3D boundaryPoint(iX,iX, iY,iY, iZ,iZ);
                  setToConstant(fields.flag, boundaryPoint, (int)freeSurfaceFlag::wall);
                }
              if (geometry.get(iX,iY, iZ) == 2) {
                  Box3D matrixPoint(iX,iX, iY,iY, iZ,iZ);
                  setToConstant(fields.flag, matrixPoint, (int)freeSurfaceFlag::wall);
                }
          }
      }
    }
