...
const int kBoundaryTag = 1;
const int kSolidTag = 2;
const int kGrayFractureTag = 3;
// Keep the fracture much more conductive than the background porous sample.
const T kGrayFractureSolidFraction = (T)0.01;

}

void addOutletSpongeZone(MultiBlockLattice3D<T,DESCRIPTOR>& lattice, T omega)
{
  const plint nx = lattice.getNx();
  const plint ny = lattice.getNy();
  const plint nz = lattice.getNz();

  plint spongeWidth = std::max<plint>(2, nx / 10);
  spongeWidth = std::min<plint>(spongeWidth, std::max<plint>(2, nx / 4));

  Array<plint,6> numSpongeCells;
  for (plint i = 0; i < 6; ++i) {
    numSpongeCells[i] = 0;
  }
  numSpongeCells[1] = spongeWidth;

  pcout << "Adding outlet sponge zone with " << spongeWidth << " cells." << std::endl;

  std::vector<MultiBlock3D*> args;
  args.push_back(&lattice);
  applyProcessingFunctional(
      new ViscositySpongeZone3D<T,DESCRIPTOR>(nx, ny, nz, omega, numSpongeCells),
...
void porousMediaSetup(MultiBlockLattice3D<T,DESCRIPTOR>& lattice,
  ...
  MultiScalarField3D<int>& geometry, T deltaP, T omega)
  {
    const plint nx = lattice.getNx();
    const plint ny = lattice.getNy();
    const plint nz = lattice.getNz();
    ...
    defineDynamics(lattice, geometry, new BounceBack<T,DESCRIPTOR>(), kBoundaryTag);
    // Where "geometry" evaluates to 2, use no-dynamics (which does nothing).
    defineDynamics(lattice, geometry, new NoDynamics<T,DESCRIPTOR>(), kSolidTag);
    // Where "geometry" evaluates to 3, use a partially saturated gray cell.
    defineDynamics(lattice, geometry, new PartialBBdynamics<T,DESCRIPTOR>(omega), kGrayFractureTag);
    setExternalScalar(
        lattice, geometry, kGrayFractureTag, lattice.getBoundingBox(),
        DESCRIPTOR<T>::ExternalField::rhoBarBeginsAt, kGrayFractureSolidFraction);
    setExternalVector(
        lattice, geometry, kGrayFractureTag, lattice.getBoundingBox(),
        DESCRIPTOR<T>::ExternalField::jBeginsAt, zeroVelocity);
  ...
  int main(int argc, char **argv)
  {
    ...
    pcout << "The convergence threshold is: " << conv << " %" << std::endl;
      ...
      defineDynamics(lattice, lattice.getBoundingBox(), new BGKdynamics<T,DESCRIPTOR>(omega));
      ...
      porousMediaSetup(lattice, createLocalBoundaryCondition3D<T,DESCRIPTOR>(),
                       geometry, deltaP, omega);
      addOutletSpongeZone(lattice, omega);
