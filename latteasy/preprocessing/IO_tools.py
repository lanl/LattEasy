...
def get_lbm_executable():
    """Return the path to the compiled ``permeability`` executable.

    Notes
    -----
    The executable is normally expected to live in ``bin/`` inside the
    ``latteasy`` package.  When running from a source checkout this function
    also checks ``src/single_phase/`` two directories above the current file so
    that examples can be executed without installing the package.
    """
...
def create_single_phase_input_file(
    input_file_name,
    geom_name,
    domain_size,
    periodic,
    ...
    with open(input_file_name, "w") as file:
        ...
        file.write(
            f"\t<per> <x> {periodic_x} </x> <y> {periodic_y} </y> <z> {periodic_z} </z> </per>\n"
        )
        file.write("</geometry>\n\n")
...
class write_MPLBM:
    ...
    def __init__(self, pore_obj, buffer_layers=2, cpus=4, num_hrs=14, allocation=None):
        ...
        ----------
            pore_obj : object
                An object containing the 3D porous geometry.
        ...
        self.folder_path = f"sims/pore_{self.folder_num}"
        create_folder(f"{self.folder_path}/input")

        # Assign folder path to geom.loc
        geom.loc = self.folder_path

        # Copy the pore object and remove 'pore_3D' to reduce pickle size
        self.my_pore = cp(pore_obj)
        ...
        # Preprocess the 3D image and generate an efficient geometry for simulation
        pore_3D = 1 - np.transpose(pore_obj, (2, 0, 1))
        pore_3D = erase_regions(pore_3D)
        pore_3D = create_geom_edist(pore_3D, geom)

        # Create input file
        pore_size = pore_3D.shape
    ...
    def create_new_folder(self):
        ...
        'pore_i' (i ranging from 0 to 9999999999) and creates the first one that doesn't already exist.
        ...
        for i in range(10000000000):
            try:
                mkdir(f"sims/pore_{i}")
                break
            except FileExistsError:
                continue
        return i

    def write_geom(self, pore):
        """
        Writes the 3D pore geometry to a .dat file.

        The function first appends the shape of the input ``pore`` array to the flattened ``pore`` array,
        then saves this information to a .dat file in the current simulation's directory.

        Parameters
        ----------
            pore : np.array
                The 3D pore geometry to be written.
        ...
        """
        tofile = np.append([*pore.shape], pore.flatten("F"))
        np.savetxt(f"{self.folder_path}/pore.dat", tofile, fmt="%.0f")
    ...
    def run_sim(self, mpi_procs=None):
        """Run the simulation and return the permeability."""
